...
*	Creates a matrix or array filled with geometric random variables.
*
...
function random( dims, options ) {
	...
	if ( ndims === 1 ) {
		...
		if ( dt === 'generic' ) {
			return array( len, p, rand );
		}
		return typedarray( len, dt, p, rand );
	}
	// Multidimensional data structures...
	if ( dt !== 'generic' ) {
		if ( ndims === 2 ) {
			return matrix( dims, dt, p, rand );
		}
		// TODO: dstructs-ndarray support goes here. Until then, fall through to plain arrays...
	}
	return arrayarray( dims, p, rand );
} // end FUNCTION random()
